...
import useLLM from '../../../hooks/useLLM'; // Import useLLM
...
const UpdateTrade = ({ id }) => {
  ...
  const handleUpdatePreview = async () => {
    ...
    const tradeData = {
      type: 'UPDATE', // Indicate this is an update
      ...
      targets: formData.target.includes('-')
        ...
        : [formData.target],
      notes: formData.notes, // Pass the notes for the LLM to work with
    };
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b sticky top-0 z-10 shadow">
        <div className="max-w-3xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <button className="text-gray-600 hover:text-gray-900" onClick={() => route('/')}>
            <X className="h-4 w-4" />
          </button>
        </div>
      </header>
